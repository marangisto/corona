...
template<>
struct peripheral_t<STM32F0x0, USB>
{
    using T = stm32f0x0_usb_t;
    static T& V;
};

template<>
struct peripheral_t<STM32F0x1, USB>
{
    using T = stm32f0x0_usb_t;
    static T& V;
};
...
template<>
struct peripheral_t<STM32F0x2, USB>
{
    using T = stm32f0x0_usb_t;
    static T& V;
};
...
template<>
struct peripheral_t<STM32F0x8, USB>
{
    using T = stm32f0x0_usb_t;
    static T& V;
};
...
template<> struct usb_traits<0>
{
    ...
    static constexpr signal_t OE = USB_OE;

    template<typename RCC>
    static void reset()
    {
        RCC::V.APB1ENR |= RCC::T::APB1ENR_USBRST;
    }
};
