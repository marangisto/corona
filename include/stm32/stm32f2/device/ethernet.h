...
struct stm32f215_ethernet_dma_t
{
    ...
    typedef bit_field_t<0, 0xff> DMARSWTR_RSWTC; // Receive status watchdog timer count
...
struct stm32f215_ethernet_mac_t
{
    ...
    typedef bit_field_t<16, 0xffff> MACFCR_PT; // Pause time

    static constexpr uint32_t MACVLANTR_RESET_VALUE = 0x0; // Reset value
    typedef bit_field_t<0, 0xffff> MACVLANTR_VLANTI; // VLAN tag identifier
    ...
    typedef bit_field_t<17, 0x3> MACDBGR_MTFCS; // MAC transmit frame controller status
    static constexpr uint32_t MACDBGR_MTP = 0x80000; // MAC transmitter in pause
    typedef bit_field_t<20, 0x3> MACDBGR_TFRS; // Tx FIFO read status
    ...
    typedef bit_field_t<0, 0xffffffff> MACA1LR_MACA1LR; // MAC address1 low

    static constexpr uint32_t MACA2HR_RESET_VALUE = 0xffff; // Reset value
    typedef bit_field_t<0, 0xffff> MACA2HR_MAC2AH; // MAC address2 high
...
struct stm32f215_ethernet_mmc_t
{
    ...
    typedef bit_field_t<0, 0xffffffff> MMCTGFSCCR_TGFSCC; // Transmitted good frames single collision counter

    static constexpr uint32_t MMCTGFMSCCR_RESET_VALUE = 0x0; // Reset value
    typedef bit_field_t<0, 0xffffffff> MMCTGFMSCCR_TGFMSCC; // Transmitted good frames more single collision counter
    ...
    typedef bit_field_t<0, 0xffffffff> MMCRFCECR_RFCFC; // Received frames CRC error counter

    static constexpr uint32_t MMCRFAECR_RESET_VALUE = 0x0; // Reset value
    typedef bit_field_t<0, 0xffffffff> MMCRFAECR_RFAEC; // Received frames alignment error counter
...
struct stm32f215_ethernet_ptp_t
{
    ...
    typedef bit_field_t<16, 0x3> PTPTSCR_TSCNT; // Time stamp clock node type
    ...
    typedef bit_field_t<0, 0xf> PTPPPSCR_PPSFREQ; // PPS frequency selection
};
...
struct peripheral_t<STM32F215, ETHERNET_DMA>
{
    ...
    using T = stm32f215_ethernet_dma_t;
    static T& V;
};
...
struct peripheral_t<STM32F215, ETHERNET_MAC>
{
    ...
    using T = stm32f215_ethernet_mac_t;
    static T& V;
};
...
{
    static constexpr periph_t P = ETHERNET_MMC;
    using T = stm32f215_ethernet_mmc_t;
    static T& V;
};
...
{
    static constexpr periph_t P = ETHERNET_PTP;
    using T = stm32f215_ethernet_ptp_t;
    static T& V;
};
