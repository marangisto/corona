...
struct stm32f410_lptim1_t
{
    ...
    volatile uint32_t ARR;  // [read-write] Autoreload Register
    volatile uint32_t CNT;  // [read-only] Counter Register
    volatile uint32_t OPTR; // [read-write] Option Register
    ...
    template<uint32_t X>
    static constexpr uint32_t OPTR_OR =   // OR
...
{
    typedef stm32f413_lptim_t T;
    static T& V;
};

using lptim_t = peripheral_t<mcu_svd, LPTIM>;
