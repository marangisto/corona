...
void (*vectors[])(void) __attribute__ ((section(".vectors"))) =
    ...
    , handler<interrupt::FSMC> // 48: FMC global interrupt
    ...
    , handler<interrupt::OTG_HS_EP1_OUT> // 74: USB On The Go HS End Point 1 Out
    , handler<interrupt::OTG_HS_EP1_IN> // 75: USB On The Go HS End Point 1 In
    , handler<interrupt::OTG_HS_WKUP> // 76: USB On The Go HS Wakeup through
    ...
    , handler<interrupt::FPU> // 81: FPU global interrupt
    ...
    , handler<interrupt::LCD_TFT_1> // 89: LCD-TFT global Error interrupt
    ...
    , handler<interrupt::HDMI_CEC> // 94: HDMI-CEC global interrupt
