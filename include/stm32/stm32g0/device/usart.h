...
{
    volatile uint32_t CR1_FIFO_ENABLED; // Control register 1
    volatile uint32_t CR2; // Control register 2
    volatile uint32_t CR3; // Control register 3
    volatile uint32_t BRR; // Baud rate register
    volatile uint32_t GTPR; // Guard time and prescaler register
    volatile uint32_t RTOR; // Receiver timeout register
    volatile uint32_t RQR; // Request register
    volatile uint32_t ISR_FIFO_ENABLED; // Interrupt &amp; status register
    volatile uint32_t ICR; // Interrupt flag clear register
    volatile uint32_t RDR; // Receive data register
    volatile uint32_t TDR; // Transmit data register
    volatile uint32_t PRESC; // Prescaler register

    static constexpr uint32_t CR1_FIFO_ENABLED_RESET_VALUE = 0x0; // Reset value
    static constexpr uint32_t CR1_FIFO_ENABLED_CMIE = 0x4000; // Character match interrupt enable This bit is set and cleared by software.
    typedef bit_field_t<21, 0x1f> CR1_FIFO_ENABLED_DEAT; // Driver Enable assertion time This 5-bit value defines the time between the activation of the DE (Driver Enable) signal and the beginning of the start bit. It is expressed in sample time units (1/8 or 1/16 bit time, depending on the oversampling rate). This bitfield can only be written when the USART is disabled (UE=0). Note: If the Driver Enable feature is not supported, this bit is reserved and must be kept at reset value. Refer to .
    typedef bit_field_t<16, 0x1f> CR1_FIFO_ENABLED_DEDT; // Driver Enable deassertion time This 5-bit value defines the time between the end of the last stop bit, in a transmitted message, and the de-activation of the DE (Driver Enable) signal. It is expressed in sample time units (1/8 or 1/16 bit time, depending on the oversampling rate). If the USART_TDR register is written during the DEDT time, the new data is transmitted only when the DEDT and DEAT times have both elapsed. This bitfield can only be written when the USART is disabled (UE=0). Note: If the Driver Enable feature is not supported, this bit is reserved and must be kept at reset value. Refer to .
    static constexpr uint32_t CR1_FIFO_ENABLED_EOBIE = 0x8000000; // End of Block interrupt enable This bit is set and cleared by software. Note: If the USART does not support Smartcard mode, this bit is reserved and must be kept at reset value. Refer to .
    static constexpr uint32_t CR1_FIFO_ENABLED_FIFOEN = 0x20000000; // FIFO mode enable This bit is set and cleared by software. This bitfield can only be written when the USART is disabled (UE=0). Note: FIFO mode can be used on standard UART communication, in SPI master/slave mode and in Smartcard modes only. It must not be enabled in IrDA and LIN modes.
    static constexpr uint32_t CR1_FIFO_ENABLED_IDLEIE = 0x10; // IDLE interrupt enable This bit is set and cleared by software.
    static constexpr uint32_t CR1_FIFO_ENABLED_M0 = 0x1000; // Word length This bit is used in conjunction with bit 28 (M1) to determine the word length. It is set or cleared by software (refer to bit 28 (M1)description). This bit can only be written when the USART is disabled (UE=0).
    static constexpr uint32_t CR1_FIFO_ENABLED_M1 = 0x10000000; // Word length This bit must be used in conjunction with bit 12 (M0) to determine the word length. It is set or cleared by software. M[1:0] = &#39;00: 1 start bit, 8 Data bits, n Stop bit M[1:0] = &#39;01: 1 start bit, 9 Data bits, n Stop bit M[1:0] = &#39;10: 1 start bit, 7 Data bits, n Stop bit This bit can only be written when the USART is disabled (UE=0). Note: In 7-bits data length mode, the Smartcard mode, LIN master mode and Auto baud rate (0x7F and 0x55 frames detection) are not supported.
    static constexpr uint32_t CR1_FIFO_ENABLED_MME = 0x2000; // Mute mode enable This bit enables the USART Mute mode function. When set, the USART can switch between active and Mute mode, as defined by the WAKE bit. It is set and cleared by software.
    static constexpr uint32_t CR1_FIFO_ENABLED_OVER8 = 0x8000; // Oversampling mode This bit can only be written when the USART is disabled (UE=0). Note: In LIN, IrDA and Smartcard modes, this bit must be kept cleared.
    static constexpr uint32_t CR1_FIFO_ENABLED_PCE = 0x400; // Parity control enable This bit selects the hardware parity control (generation and detection). When the parity control is enabled, the computed parity is inserted at the MSB position (9th bit if M=1; 8th bit if M=0) and the parity is checked on the received data. This bit is set and cleared by software. Once it is set, PCE is active after the current byte (in reception and in transmission). This bitfield can only be written when the USART is disabled (UE=0).
    static constexpr uint32_t CR1_FIFO_ENABLED_PEIE = 0x100; // PE interrupt enable This bit is set and cleared by software.
    static constexpr uint32_t CR1_FIFO_ENABLED_PS = 0x200; // Parity selection This bit selects the odd or even parity when the parity generation/detection is enabled (PCE bit set). It is set and cleared by software. The parity is selected after the current byte. This bitfield can only be written when the USART is disabled (UE=0).
    static constexpr uint32_t CR1_FIFO_ENABLED_RE = 0x4; // Receiver enable This bit enables the receiver. It is set and cleared by software.
    static constexpr uint32_t CR1_FIFO_ENABLED_RTOIE = 0x4000000; // Receiver timeout interrupt enable This bit is set and cleared by software. Note: If the USART does not support the Receiver timeout feature, this bit is reserved and must be kept at reset value. .
    static constexpr uint32_t CR1_FIFO_ENABLED_RXFFIE = 0x80000000; // RXFIFO Full interrupt enable This bit is set and cleared by software.
    static constexpr uint32_t CR1_FIFO_ENABLED_RXFNEIE = 0x20; // RXFIFO not empty interrupt enable This bit is set and cleared by software.
    static constexpr uint32_t CR1_FIFO_ENABLED_RXNEIE = 0x20; // Receive data register not empty This bit is set and cleared by software.
    static constexpr uint32_t CR1_FIFO_ENABLED_TCIE = 0x40; // Transmission complete interrupt enable This bit is set and cleared by software.
    static constexpr uint32_t CR1_FIFO_ENABLED_TE = 0x8; // Transmitter enable This bit enables the transmitter. It is set and cleared by software. Note: During transmission, a low pulse on the TE bit (&#39;0 followed by &#39;1) sends a preamble (idle line) after the current word, except in Smartcard mode. In order to generate an idle character, the TE must not be immediately written to &#39;1. To ensure the required duration, the software can poll the TEACK bit in the USART_ISR register. In Smartcard mode, when TE is set, there is a 1 bit-time delay before the transmission starts.
    static constexpr uint32_t CR1_FIFO_ENABLED_TXEIE = 0x80; // Transmit data register empty This bit is set and cleared by software.
    static constexpr uint32_t CR1_FIFO_ENABLED_TXFEIE = 0x40000000; // TXFIFO empty interrupt enable This bit is set and cleared by software.
    static constexpr uint32_t CR1_FIFO_ENABLED_TXFNFIE = 0x80; // TXFIFO not full interrupt enable This bit is set and cleared by software.
    static constexpr uint32_t CR1_FIFO_ENABLED_UE = 0x1; // USART enable When this bit is cleared, the USART prescalers and outputs are stopped immediately, and all current operations are discarded. The USART configuration is kept, but all the USART_ISR status flags are reset. This bit is set and cleared by software. Note: To enter low-power mode without generating errors on the line, the TE bit must be previously reset and the software must wait for the TC bit in the USART_ISR to be set before resetting the UE bit. The DMA requests are also reset when UE = 0 so the DMA channel must be disabled before resetting the UE bit. In Smartcard mode, (SCEN = 1), the SCLK is always available when CLKEN = 1, regardless of the UE bit value.
    static constexpr uint32_t CR1_FIFO_ENABLED_UESM = 0x2; // USART enable in low-power mode When this bit is cleared, the USART cannot wake up the MCU from low-power mode. When this bit is set, the USART can wake up the MCU from low-power mode. This bit is set and cleared by software. Note: It is recommended to set the UESM bit just before entering low-power mode and clear it when exit from low-power mode. If the USART does not support the wakeup from Stop feature, this bit is reserved and must be kept at reset value. Refer to .
    static constexpr uint32_t CR1_FIFO_ENABLED_WAKE = 0x800; // Receiver wakeup method This bit determines the USART wakeup method from Mute mode. It is set or cleared by software. This bitfield can only be written when the USART is disabled (UE=0).
