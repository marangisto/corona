...
////
//
//      STM32H7 CEC peripherals
//
///

// CEC: CEC

struct stm32h723_cec_t
{
    volatile uint32_t CR; // CEC control register
    volatile uint32_t CFGR; // This register is used to configure the HDMI-CEC controller. It is mandatory to write CEC_CFGR only when CECEN=0.
    volatile uint32_t TXDR; // CEC Tx data register
    volatile uint32_t RXDR; // CEC Rx Data Register
    volatile uint32_t ISR; // CEC Interrupt and Status Register
    volatile uint32_t IER; // CEC interrupt enable register
    ...
    typedef bit_field_t<0, 0xff> TXDR_TXD; // Tx Data register. TXD is a write-only register containing the data byte to be transmitted. Note: TXD must be written when TXSTART=1

    static constexpr uint32_t RXDR_RESET_VALUE = 0x0; // Reset value
    typedef bit_field_t<0, 0xff> RXDR_RXD; // Rx Data register. RXD is read-only and contains the last data byte which has been received from the CEC line.
...
template<>
struct peripheral_t<STM32H723, HDMI_CEC>
{
    static constexpr periph_t P = HDMI_CEC;
    using T = stm32h723_cec_t;
    static T& V;
};

template<>
struct peripheral_t<STM32H725, HDMI_CEC>
{
    static constexpr periph_t P = HDMI_CEC;
    using T = stm32h723_cec_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H73x, HDMI_CEC>
{
    static constexpr periph_t P = HDMI_CEC;
    using T = stm32h723_cec_t;
    static T& V;
