...
struct stm32h742x_cec_t
{
    ...
    template<uint32_t X>
    static constexpr uint32_t TXDR_TXD =   // Tx Data register. TXD is a write-only register containing the data byte to be transmitted. Note: TXD must be written when TXSTART=1
        bit_field_t<0, 0xff>::value<X>();

    static constexpr uint32_t RXDR_RESET_VALUE = 0x0;
    template<uint32_t X>
    static constexpr uint32_t RXDR_RXD =   // Rx Data register. RXD is read-only and contains the last data byte which has been received from the CEC line.
        bit_field_t<0, 0xff>::value<X>();
...
struct stm32h750x_cec_t
{
    ...
    template<uint32_t X>
    static constexpr uint32_t CEC_TXDR_TXD =   // Tx Data register. TXD is a write-only register containing the data byte to be transmitted. Note: TXD must be written when TXSTART=1
        bit_field_t<0, 0xff>::value<X>();

    static constexpr uint32_t CEC_RXDR_RESET_VALUE = 0x0;
    template<uint32_t X>
    static constexpr uint32_t CEC_RXDR_RXD =   // Rx Data register. RXD is read-only and contains the last data byte which has been received from the CEC line.
        bit_field_t<0, 0xff>::value<X>();
