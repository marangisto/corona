...
struct stm32h742x_crs_t
{
    ...
    template<uint32_t X>
    static constexpr uint32_t ISR_FECAP =           // Frequency error capture FECAP is the frequency error counter value latched in the time of the last SYNC event. Refer to Section7.3.4: Frequency error evaluation and automatic trimming for more details about FECAP usage.
...
struct stm32h750x_crs_t
{
    ...
    volatile uint32_t CRS_CFGR; // [read-write] This register can be written only when the frequency error counter is disabled (CEN bit is cleared in CRS_CR). When the counter is enabled, this register is write-protected.
    volatile uint32_t CRS_ISR;  // [read-only] CRS interrupt and status register
    volatile uint32_t CRS_ICR;  // [read-write] CRS interrupt flag clear register

    static constexpr uint32_t CRS_CR_RESET_VALUE = 0x2000;
    ...
    static constexpr uint32_t CRS_CFGR_RESET_VALUE = 0x2022bb7f;
    template<uint32_t X>
    static constexpr uint32_t CRS_CFGR_RELOAD =              // Counter reload value RELOAD is the value to be loaded in the frequency error counter with each SYNC event. Refer to Section7.3.3: Frequency error measurement for more details about counter behavior.
        bit_field_t<0, 0xffff>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t CRS_CFGR_FELIM =               // Frequency error limit FELIM contains the value to be used to evaluate the captured frequency error value latched in the FECAP[15:0] bits of the CRS_ISR register. Refer to Section7.3.4: Frequency error evaluation and automatic trimming for more details about FECAP evaluation.
        bit_field_t<16, 0xff>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t CRS_CFGR_SYNCDIV =             // SYNC divider These bits are set and cleared by software to control the division factor of the SYNC signal.
        bit_field_t<24, 0x7>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t CRS_CFGR_SYNCSRC =             // SYNC signal source selection These bits are set and cleared by software to select the SYNC signal source. Note: When using USB LPM (Link Power Management) and the device is in Sleep mode, the periodic USB SOF will not be generated by the host. No SYNC signal will therefore be provided to the CRS to calibrate the HSI48 on the run. To guarantee the required clock precision after waking up from Sleep mode, the LSE or reference clock on the GPIOs should be used as SYNC signal.
        bit_field_t<28, 0x3>::value<X>();
    ...
    static constexpr uint32_t CRS_ISR_RESET_VALUE = 0x0;
    ...
    template<uint32_t X>
    static constexpr uint32_t CRS_ISR_FECAP =           // Frequency error capture FECAP is the frequency error counter value latched in the time of the last SYNC event. Refer to Section7.3.4: Frequency error evaluation and automatic trimming for more details about FECAP usage.
        bit_field_t<16, 0xffff>::value<X>();

    static constexpr uint32_t CRS_ICR_RESET_VALUE = 0x0;
