...
struct stm32h742x_i2c1_t
{
    ...
    template<uint32_t X>
    static constexpr uint32_t CR2_NBYTES =             // Number of bytes The number of bytes to be transmitted/received is programmed there. This field is dont care in slave mode with SBC=0. Note: Changing these bits when the START bit is set is not allowed.
    ...
    template<uint32_t X>
    static constexpr uint32_t OAR2_OA2 =           // Interface address bits 7:1 of address Note: These bits can be written only when OA2EN=0.
        bit_field_t<1, 0x7f>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t OAR2_OA2MSK =        // Own Address 2 masks Note: These bits can be written only when OA2EN=0. As soon as OA2MSK is not equal to 0, the reserved I2C addresses (0b0000xxx and 0b1111xxx) are not acknowledged even if the comparison matches.
    ...
    template<uint32_t X>
    static constexpr uint32_t TIMINGR_SCLL =     // SCL low period (master mode) This field is used to generate the SCL low period in master mode. tSCLL = (SCLL+1) x tPRESC Note: SCLL is also used to generate tBUF and tSU:STA timings.
        bit_field_t<0, 0xff>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t TIMINGR_SCLH =     // SCL high period (master mode) This field is used to generate the SCL high period in master mode. tSCLH = (SCLH+1) x tPRESC Note: SCLH is also used to generate tSU:STO and tHD:STA timing.
    ...
    template<uint32_t X>
    static constexpr uint32_t ISR_ADDCODE =         // Address match code (Slave mode) These bits are updated with the received address when an address match event occurs (ADDR = 1). In the case of a 10-bit address, ADDCODE provides the 10-bit header followed by the 2 MSBs of the address.
    ...
    template<uint32_t X>
    static constexpr uint32_t PECR_PEC =   // Packet error checking register This field contains the internal PEC when PECEN=1. The PEC is cleared by hardware when PE=0.
        bit_field_t<0, 0xff>::value<X>();

    static constexpr uint32_t RXDR_RESET_VALUE = 0x0;
    template<uint32_t X>
    static constexpr uint32_t RXDR_RXDATA =   // 8-bit receive data Data byte received from the I2C bus.
        bit_field_t<0, 0xff>::value<X>();

    static constexpr uint32_t TXDR_RESET_VALUE = 0x0;
    template<uint32_t X>
    static constexpr uint32_t TXDR_TXDATA =   // 8-bit transmit data Data byte to be transmitted to the I2C bus. Note: These bits can be written only when TXE=1.
        bit_field_t<0, 0xff>::value<X>();
};
...
{
    volatile uint32_t I2C_CR1;      // [read-write] Access: No wait states, except if a write access occurs while a write access to this register is ongoing. In this case, wait states are inserted in the second write access until the previous one is completed. The latency of the second write access can be up to 2 x PCLK1 + 6 x I2CCLK.
    volatile uint32_t I2C_CR2;      // [read-write] Access: No wait states, except if a write access occurs while a write access to this register is ongoing. In this case, wait states are inserted in the second write access until the previous one is completed. The latency of the second write access can be up to 2 x PCLK1 + 6 x I2CCLK.
    volatile uint32_t I2C_OAR1;     // [read-write] Access: No wait states, except if a write access occurs while a write access to this register is ongoing. In this case, wait states are inserted in the second write access until the previous one is completed. The latency of the second write access can be up to 2 x PCLK1 + 6 x I2CCLK.
    volatile uint32_t I2C_OAR2;     // [read-write] Access: No wait states, except if a write access occurs while a write access to this register is ongoing. In this case, wait states are inserted in the second write access until the previous one is completed. The latency of the second write access can be up to 2 x PCLK1 + 6 x I2CCLK.
    volatile uint32_t I2C_TIMINGR;  // [read-write] Access: No wait states
    volatile uint32_t I2C_TIMEOUTR; // [read-write] Access: No wait states, except if a write access occurs while a write access to this register is ongoing. In this case, wait states are inserted in the second write access until the previous one is completed. The latency of the second write access can be up to 2 x PCLK1 + 6 x I2CCLK.
    ...
    template<uint32_t X>
    static constexpr uint32_t I2C_CR1_DNF =                // Digital noise filter These bits are used to configure the digital noise filter on SDA and SCL input. The digital filter will filter spikes with a length of up to DNF[3:0] * tI2CCLK ... Note: If the analog filter is also enabled, the digital filter is added to the analog filter. This filter can only be programmed when the I2C is disabled (PE = 0).
        bit_field_t<8, 0xf>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t I2C_CR2_NBYTES =             // Number of bytes The number of bytes to be transmitted/received is programmed there. This field is dont care in slave mode with SBC=0. Note: Changing these bits when the START bit is set is not allowed.
        bit_field_t<16, 0xff>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t I2C_OAR1_OA1 =            // Interface address 7-bit addressing mode: dont care 10-bit addressing mode: bits 9:8 of address Note: These bits can be written only when OA1EN=0. OA1[7:1]: Interface address Bits 7:1 of address Note: These bits can be written only when OA1EN=0. OA1[0]: Interface address 7-bit addressing mode: dont care 10-bit addressing mode: bit 0 of address Note: This bit can be written only when OA1EN=0.
        bit_field_t<0, 0x3ff>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t I2C_OAR2_OA2 =           // Interface address bits 7:1 of address Note: These bits can be written only when OA2EN=0.
        bit_field_t<1, 0x7f>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t I2C_OAR2_OA2MSK =        // Own Address 2 masks Note: These bits can be written only when OA2EN=0. As soon as OA2MSK is not equal to 0, the reserved I2C addresses (0b0000xxx and 0b1111xxx) are not acknowledged even if the comparison matches.
        bit_field_t<8, 0x7>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t I2C_TIMINGR_SCLL =     // SCL low period (master mode) This field is used to generate the SCL low period in master mode. tSCLL = (SCLL+1) x tPRESC Note: SCLL is also used to generate tBUF and tSU:STA timings.
        bit_field_t<0, 0xff>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t I2C_TIMINGR_SCLH =     // SCL high period (master mode) This field is used to generate the SCL high period in master mode. tSCLH = (SCLH+1) x tPRESC Note: SCLH is also used to generate tSU:STO and tHD:STA timing.
        bit_field_t<8, 0xff>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t I2C_TIMINGR_SDADEL =   // Data hold time This field is used to generate the delay tSDADEL between SCL falling edge and SDA edge. In master mode and in slave mode with NOSTRETCH = 0, the SCL line is stretched low during tSDADEL. tSDADEL= SDADEL x tPRESC Note: SDADEL is used to generate tHD:DAT timing.
        bit_field_t<16, 0xf>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t I2C_TIMINGR_SCLDEL =   // Data setup time This field is used to generate a delay tSCLDEL between SDA edge and SCL rising edge. In master mode and in slave mode with NOSTRETCH = 0, the SCL line is stretched low during tSCLDEL. tSCLDEL = (SCLDEL+1) x tPRESC Note: tSCLDEL is used to generate tSU:DAT timing.
        bit_field_t<20, 0xf>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t I2C_TIMINGR_PRESC =    // Timing prescaler This field is used to prescale I2CCLK in order to generate the clock period tPRESC used for data setup and hold counters (refer to I2C timings on page9) and for SCL high and low level counters (refer to I2C master initialization on page24). tPRESC = (PRESC+1) x tI2CCLK
        bit_field_t<28, 0xf>::value<X>();

    static constexpr uint32_t I2C_TIMEOUTR_RESET_VALUE = 0x0;
    template<uint32_t X>
    static constexpr uint32_t I2C_TIMEOUTR_TIMEOUTA =           // Bus Timeout A This field is used to configure: The SCL low timeout condition tTIMEOUT when TIDLE=0 tTIMEOUT= (TIMEOUTA+1) x 2048 x tI2CCLK The bus idle condition (both SCL and SDA high) when TIDLE=1 tIDLE= (TIMEOUTA+1) x 4 x tI2CCLK Note: These bits can be written only when TIMOUTEN=0.
        bit_field_t<0, 0xfff>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t I2C_TIMEOUTR_TIMEOUTB =           // Bus timeout B This field is used to configure the cumulative clock extension timeout: In master mode, the master cumulative clock low extend time (tLOW:MEXT) is detected In slave mode, the slave cumulative clock low extend time (tLOW:SEXT) is detected tLOW:EXT= (TIMEOUTB+1) x 2048 x tI2CCLK Note: These bits can be written only when TEXTEN=0.
        bit_field_t<16, 0xfff>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t I2C_ISR_ADDCODE =         // Address match code (Slave mode) These bits are updated with the received address when an address match event occurs (ADDR = 1). In the case of a 10-bit address, ADDCODE provides the 10-bit header followed by the 2 MSBs of the address.
        bit_field_t<17, 0x7f>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t I2C_PECR_PEC =   // Packet error checking register This field contains the internal PEC when PECEN=1. The PEC is cleared by hardware when PE=0.
        bit_field_t<0, 0xff>::value<X>();

    static constexpr uint32_t I2C_RXDR_RESET_VALUE = 0x0;
    template<uint32_t X>
    static constexpr uint32_t I2C_RXDR_RXDATA =   // 8-bit receive data Data byte received from the I2C bus.
        bit_field_t<0, 0xff>::value<X>();

    static constexpr uint32_t I2C_TXDR_RESET_VALUE = 0x0;
    template<uint32_t X>
    static constexpr uint32_t I2C_TXDR_TXDATA =   // 8-bit transmit data Data byte to be transmitted to the I2C bus. Note: These bits can be written only when TXE=1.
        bit_field_t<0, 0xff>::value<X>();
};
