...
struct stm32h750x_pwr_t
{
    ...
    reserved_t<0x1> _0x1c;
    volatile uint32_t PWR_WKUPCR; // reset only by system reset, not reset by wakeup from Standby mode5 wait states are required when writing this register (when clearing a WKUPF bit in PWR_WKUPFR, the AHB write access will complete after the WKUPF has been cleared).
    volatile uint32_t PWR_WKUPFR; // reset only by system reset, not reset by wakeup from Standby mode
    volatile uint32_t PWR_WKUPEPR; // Reset only by system reset, not reset by wakeup from Standby mode

    static constexpr uint32_t PWR_CR1_RESET_VALUE = 0xf000c000; // Reset value
    ...
    typedef bit_field_t<5, 0x7> PWR_CR1_PLS; // Programmable voltage detector level selection These bits select the voltage threshold detected by the PVD. Note: Refer to Section Electrical characteristics of the product datasheet for more details.
    ...
    typedef bit_field_t<14, 0x3> PWR_CR1_SVOS; // System Stop mode voltage scaling selection These bits control the VCORE voltage level in system Stop mode, to obtain the best trade-off between power consumption and performance.
    static constexpr uint32_t PWR_CR1_AVDEN = 0x10000; // Peripheral voltage monitor on VDDA enable
    typedef bit_field_t<17, 0x3> PWR_CR1_ALS; // Analog voltage detector level selection These bits select the voltage threshold detected by the AVD.

    static constexpr uint32_t PWR_CSR1_RESET_VALUE = 0x4000; // Reset value
    ...
    typedef bit_field_t<14, 0x3> PWR_CSR1_ACTVOS; // VOS currently applied for VCORE voltage scaling selection. These bits reflect the last VOS value applied to the PMU.
    ...
    static constexpr uint32_t PWR_CR3_RESET_VALUE = 0x6; // Reset value
    ...
    static constexpr uint32_t PWR_CPUCR_RESET_VALUE = 0x0; // Reset value
    ...
    static constexpr uint32_t PWR_D3CR_RESET_VALUE = 0x4000; // Reset value
    ...
    typedef bit_field_t<14, 0x3> PWR_D3CR_VOS; // Voltage scaling selection according to performance These bits control the VCORE voltage level and allow to obtains the best trade-off between power consumption and performance: When increasing the performance, the voltage scaling shall be changed before increasing the system frequency. When decreasing performance, the system frequency shall first be decreased before changing the voltage scaling.


    static constexpr uint32_t PWR_WKUPCR_RESET_VALUE = 0x0; // Reset value
    typedef bit_field_t<0, 0x3f> PWR_WKUPCR_WKUPC; // Clear Wakeup pin flag for WKUP. These bits are always read as 0.

    static constexpr uint32_t PWR_WKUPFR_RESET_VALUE = 0x0; // Reset value
    ...
    static constexpr uint32_t PWR_WKUPEPR_RESET_VALUE = 0x0; // Reset value
    ...
    typedef bit_field_t<16, 0x3> PWR_WKUPEPR_WKUPPUPD1; // Wakeup pin pull configuration
    typedef bit_field_t<18, 0x3> PWR_WKUPEPR_WKUPPUPD2; // Wakeup pin pull configuration
    typedef bit_field_t<20, 0x3> PWR_WKUPEPR_WKUPPUPD3; // Wakeup pin pull configuration
    typedef bit_field_t<22, 0x3> PWR_WKUPEPR_WKUPPUPD4; // Wakeup pin pull configuration
    typedef bit_field_t<24, 0x3> PWR_WKUPEPR_WKUPPUPD5; // Wakeup pin pull configuration
    typedef bit_field_t<26, 0x3> PWR_WKUPEPR_WKUPPUPD6; // Wakeup pin pull configuration for WKUP(truncate(n/2)-7) These bits define the I/O pad pull configuration used when WKUPEN(truncate(n/2)-7) = 1. The associated GPIO port pull configuration shall be set to the same value or to 00. The Wakeup pin pull configuration is kept in Standby mode.
};
