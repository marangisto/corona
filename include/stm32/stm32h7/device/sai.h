...
struct stm32h742x_sai4_t
{
    ...
    volatile uint32_t SAI_ASLOTR; // [read-write] This register has no meaning in AC97 and SPDIF audio protocol
    volatile uint32_t SAI_AIM;    // [read-write] Interrupt mask register 2
    ...
    volatile uint32_t SAI_BSLOTR; // [read-write] This register has no meaning in AC97 and SPDIF audio protocol
    volatile uint32_t SAI_BIM;    // [read-write] Interrupt mask register 2
    ...
    template<uint32_t X>
    static constexpr uint32_t SAI_GCR_SYNCOUT =   // Synchronization outputs These bits are set and cleared by software.
        bit_field_t<4, 0x3>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_GCR_SYNCIN =    // Synchronization inputs
        bit_field_t<0, 0x3>::value<X>();

    static constexpr uint32_t SAI_ACR1_RESET_VALUE = 0x40;
    template<uint32_t X>
    static constexpr uint32_t SAI_ACR1_MODE =            // SAIx audio block mode immediately
        bit_field_t<0, 0x3>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_ACR1_PRTCFG =          // Protocol configuration. These bits are set and cleared by software. These bits have to be configured when the audio block is disabled.
        bit_field_t<2, 0x3>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_ACR1_DS =              // Data size. These bits are set and cleared by software. These bits are ignored when the SPDIF protocols are selected (bit PRTCFG[1:0]), because the frame and the data size are fixed in such case. When the companding mode is selected through COMP[1:0] bits, DS[1:0] are ignored since the data size is fixed to 8 bits by the algorithm. These bits must be configured when the audio block is disabled.
    ...
    template<uint32_t X>
    static constexpr uint32_t SAI_ACR1_SYNCEN =          // Synchronization enable. These bits are set and cleared by software. They must be configured when the audio sub-block is disabled. Note: The audio sub-block should be configured as asynchronous when SPDIF mode is enabled.
    ...
    template<uint32_t X>
    static constexpr uint32_t SAI_ACR1_MCKDIV =          // Master clock divider. These bits are set and cleared by software. These bits are meaningless when the audio block operates in slave mode. They have to be configured when the audio block is disabled. Others: the master clock frequency is calculated accordingly to the following formula:
    ...
    template<uint32_t X>
    static constexpr uint32_t SAI_ACR2_FTH =           // FIFO threshold. This bit is set and cleared by software.
    ...
    template<uint32_t X>
    static constexpr uint32_t SAI_ACR2_MUTECNT =       // Mute counter. These bits are set and cleared by software. They are used only in reception mode. The value set in these bits is compared to the number of consecutive mute frames detected in reception. When the number of mute frames is equal to this value, the flag MUTEDET will be set and an interrupt will be generated if bit MUTEDETIE is set. Refer to Section: Mute mode for more details.
    ...
    template<uint32_t X>
    static constexpr uint32_t SAI_ACR2_COMP =          // Companding mode. These bits are set and cleared by software. The -Law and the A-Law log are a part of the CCITT G.711 recommendation, the type of complement that will be used depends on CPL bit. The data expansion or data compression are determined by the state of bit MODE[0]. The data compression is applied if the audio block is configured as a transmitter. The data expansion is automatically applied when the audio block is configured as a receiver. Refer to Section: Companding mode for more details. Note: Companding mode is applicable only when TDM is selected.
        bit_field_t<14, 0x3>::value<X>();

    static constexpr uint32_t SAI_AFRCR_RESET_VALUE = 0x7;
    template<uint32_t X>
    static constexpr uint32_t SAI_AFRCR_FRL =            // Frame length. These bits are set and cleared by software. They define the audio frame length expressed in number of SCK clock cycles: the number of bits in the frame is equal to FRL[7:0] + 1. The minimum number of bits to transfer in an audio frame must be equal to 8, otherwise the audio block will behaves in an unexpected way. This is the case when the data size is 8 bits and only one slot 0 is defined in NBSLOT[4:0] of SAI_xSLOTR register (NBSLOT[3:0] = 0000). In master mode, if the master clock (available on MCLK_x pin) is used, the frame length should be aligned with a number equal to a power of 2, ranging from 8 to 256. When the master clock is not used (NODIV = 1), it is recommended to program the frame length to an value ranging from 8 to 256. These bits are meaningless and are not used in AC97 or SPDIF audio block configuration.
        bit_field_t<0, 0xff>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_AFRCR_FSALL =          // Frame synchronization active level length. These bits are set and cleared by software. They specify the length in number of bit clock (SCK) + 1 (FSALL[6:0] + 1) of the active level of the FS signal in the audio frame These bits are meaningless and are not used in AC97 or SPDIF audio block configuration. They must be configured when the audio block is disabled.
    ...
    template<uint32_t X>
    static constexpr uint32_t SAI_ASLOTR_FBOFF =    // First bit offset These bits are set and cleared by software. The value set in this bitfield defines the position of the first data transfer bit in the slot. It represents an offset value. In transmission mode, the bits outside the data field are forced to 0. In reception mode, the extra received bits are discarded. These bits must be set when the audio block is disabled. They are ignored in AC97 or SPDIF mode.
        bit_field_t<0, 0x1f>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_ASLOTR_SLOTSZ =   // Slot size This bits is set and cleared by software. The slot size must be higher or equal to the data size. If this condition is not respected, the behavior of the SAI will be undetermined. Refer to Section: Output data line management on an inactive slot for information on how to drive SD line. These bits must be set when the audio block is disabled. They are ignored in AC97 or SPDIF mode.
        bit_field_t<6, 0x3>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_ASLOTR_NBSLOT =   // Number of slots in an audio frame. These bits are set and cleared by software. The value set in this bitfield represents the number of slots + 1 in the audio frame (including the number of inactive slots). The maximum number of slots is 16. The number of slots should be even if FSDEF bit in the SAI_xFRCR register is set. The number of slots must be configured when the audio block is disabled. They are ignored in AC97 or SPDIF mode.
        bit_field_t<8, 0xf>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_ASLOTR_SLOTEN =   // Slot enable. These bits are set and cleared by software. Each SLOTEN bit corresponds to a slot position from 0 to 15 (maximum 16 slots). The slot must be enabled when the audio block is disabled. They are ignored in AC97 or SPDIF mode.
    ...
    template<uint32_t X>
    static constexpr uint32_t SAI_BCR1_MODE =            // SAIx audio block mode immediately
        bit_field_t<0, 0x3>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_BCR1_PRTCFG =          // Protocol configuration. These bits are set and cleared by software. These bits have to be configured when the audio block is disabled.
        bit_field_t<2, 0x3>::value<X>();
    template<uint32_t X>
    ...
    template<uint32_t X>
    static constexpr uint32_t SAI_BCR1_SYNCEN =          // Synchronization enable. These bits are set and cleared by software. They must be configured when the audio sub-block is disabled. Note: The audio sub-block should be configured as asynchronous when SPDIF mode is enabled.
        bit_field_t<10, 0x3>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t SAI_BCR1_MCKDIV =          // Master clock divider. These bits are set and cleared by software. These bits are meaningless when the audio block operates in slave mode. They have to be configured when the audio block is disabled. Others: the master clock frequency is calculated accordingly to the following formula:
        bit_field_t<20, 0xf>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t SAI_BCR2_FTH =           // FIFO threshold. This bit is set and cleared by software.
        bit_field_t<0, 0x7>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t SAI_BCR2_MUTECNT =       // Mute counter. These bits are set and cleared by software. They are used only in reception mode. The value set in these bits is compared to the number of consecutive mute frames detected in reception. When the number of mute frames is equal to this value, the flag MUTEDET will be set and an interrupt will be generated if bit MUTEDETIE is set. Refer to Section: Mute mode for more details.
        bit_field_t<7, 0x3f>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t SAI_BCR2_COMP =          // Companding mode. These bits are set and cleared by software. The -Law and the A-Law log are a part of the CCITT G.711 recommendation, the type of complement that will be used depends on CPL bit. The data expansion or data compression are determined by the state of bit MODE[0]. The data compression is applied if the audio block is configured as a transmitter. The data expansion is automatically applied when the audio block is configured as a receiver. Refer to Section: Companding mode for more details. Note: Companding mode is applicable only when TDM is selected.
        bit_field_t<14, 0x3>::value<X>();

    static constexpr uint32_t SAI_BFRCR_RESET_VALUE = 0x7;
    template<uint32_t X>
    static constexpr uint32_t SAI_BFRCR_FRL =            // Frame length. These bits are set and cleared by software. They define the audio frame length expressed in number of SCK clock cycles: the number of bits in the frame is equal to FRL[7:0] + 1. The minimum number of bits to transfer in an audio frame must be equal to 8, otherwise the audio block will behaves in an unexpected way. This is the case when the data size is 8 bits and only one slot 0 is defined in NBSLOT[4:0] of SAI_xSLOTR register (NBSLOT[3:0] = 0000). In master mode, if the master clock (available on MCLK_x pin) is used, the frame length should be aligned with a number equal to a power of 2, ranging from 8 to 256. When the master clock is not used (NODIV = 1), it is recommended to program the frame length to an value ranging from 8 to 256. These bits are meaningless and are not used in AC97 or SPDIF audio block configuration.
        bit_field_t<0, 0xff>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_BFRCR_FSALL =          // Frame synchronization active level length. These bits are set and cleared by software. They specify the length in number of bit clock (SCK) + 1 (FSALL[6:0] + 1) of the active level of the FS signal in the audio frame These bits are meaningless and are not used in AC97 or SPDIF audio block configuration. They must be configured when the audio block is disabled.
        bit_field_t<8, 0x7f>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t SAI_BSLOTR_FBOFF =    // First bit offset These bits are set and cleared by software. The value set in this bitfield defines the position of the first data transfer bit in the slot. It represents an offset value. In transmission mode, the bits outside the data field are forced to 0. In reception mode, the extra received bits are discarded. These bits must be set when the audio block is disabled. They are ignored in AC97 or SPDIF mode.
        bit_field_t<0, 0x1f>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_BSLOTR_SLOTSZ =   // Slot size This bits is set and cleared by software. The slot size must be higher or equal to the data size. If this condition is not respected, the behavior of the SAI will be undetermined. Refer to Section: Output data line management on an inactive slot for information on how to drive SD line. These bits must be set when the audio block is disabled. They are ignored in AC97 or SPDIF mode.
        bit_field_t<6, 0x3>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_BSLOTR_NBSLOT =   // Number of slots in an audio frame. These bits are set and cleared by software. The value set in this bitfield represents the number of slots + 1 in the audio frame (including the number of inactive slots). The maximum number of slots is 16. The number of slots should be even if FSDEF bit in the SAI_xFRCR register is set. The number of slots must be configured when the audio block is disabled. They are ignored in AC97 or SPDIF mode.
        bit_field_t<8, 0xf>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_BSLOTR_SLOTEN =   // Slot enable. These bits are set and cleared by software. Each SLOTEN bit corresponds to a slot position from 0 to 15 (maximum 16 slots). The slot must be enabled when the audio block is disabled. They are ignored in AC97 or SPDIF mode.
        bit_field_t<16, 0xffff>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t SAI_PDMCR_MICNBR =       // Number of microphones
        bit_field_t<4, 0x3>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t SAI_PDMDLY_DLYM1L =   // Delay line adjust for first microphone of pair 1
        bit_field_t<0, 0x7>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_PDMDLY_DLYM1R =   // Delay line adjust for second microphone of pair 1
        bit_field_t<4, 0x7>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_PDMDLY_DLYM2L =   // Delay line for first microphone of pair 2
        bit_field_t<8, 0x7>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_PDMDLY_DLYM2R =   // Delay line for second microphone of pair 2
        bit_field_t<12, 0x7>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_PDMDLY_DLYM3L =   // Delay line for first microphone of pair 3
        bit_field_t<16, 0x7>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_PDMDLY_DLYM3R =   // Delay line for second microphone of pair 3
        bit_field_t<20, 0x7>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_PDMDLY_DLYM4L =   // Delay line for first microphone of pair 4
        bit_field_t<24, 0x7>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SAI_PDMDLY_DLYM4R =   // Delay line for second microphone of pair 4
