...
////
//
//      STM32H7 SDMMC peripherals
//
///

// SDMMC1: SDMMC1

struct stm32h723_sdmmc1_t
{
...
struct stm32h742x_sdmmc1_t
{
    volatile uint32_t POWER; // SDMMC power control register
    volatile uint32_t CLKCR; // The SDMMC_CLKCR register controls the SDMMC_CK output clock, the SDMMC_RX_CLK receive clock, and the bus width.
    volatile uint32_t ARGR; // The SDMMC_ARGR register contains a 32-bit command argument, which is sent to a card as part of a command message.
    volatile uint32_t CMDR; // The SDMMC_CMDR register contains the command index and command type bits. The command index is sent to a card as part of a command message. The command type bits control the command path state machine (CPSM).
    volatile uint32_t RESPCMDR; // SDMMC command response register
    volatile uint32_t RESP1R; // The SDMMC_RESP1/2/3/4R registers contain the status of a card, which is part of the received response.
    volatile uint32_t RESP2R; // The SDMMC_RESP1/2/3/4R registers contain the status of a card, which is part of the received response.
    volatile uint32_t RESP3R; // The SDMMC_RESP1/2/3/4R registers contain the status of a card, which is part of the received response.
    volatile uint32_t RESP4R; // The SDMMC_RESP1/2/3/4R registers contain the status of a card, which is part of the received response.
    volatile uint32_t DTIMER; // The SDMMC_DTIMER register contains the data timeout period, in card bus clock periods. A counter loads the value from the SDMMC_DTIMER register, and starts decrementing when the data path state machine (DPSM) enters the Wait_R or Busy state. If the timer reaches 0 while the DPSM is in either of these states, the timeout status flag is set.
    volatile uint32_t DLENR; // The SDMMC_DLENR register contains the number of data bytes to be transferred. The value is loaded into the data counter when data transfer starts.
    volatile uint32_t DCTRL; // The SDMMC_DCTRL register control the data path state machine (DPSM).
    volatile uint32_t DCNTR; // The SDMMC_DCNTR register loads the value from the data length register (see SDMMC_DLENR) when the DPSM moves from the Idle state to the Wait_R or Wait_S state. As data is transferred, the counter decrements the value until it reaches 0. The DPSM then moves to the Idle state and when there has been no error, the data status end flag (DATAEND) is set.
    volatile uint32_t STAR; // The SDMMC_STAR register is a read-only register. It contains two types of flag:Static flags (bits [29,21,11:0]): these bits remain asserted until they are cleared by writing to the SDMMC interrupt Clear register (see SDMMC_ICR)Dynamic flags (bits [20:12]): these bits change state depending on the state of the underlying logic (for example, FIFO full and empty flags are asserted and de-asserted as data while written to the FIFO)
    volatile uint32_t ICR; // The SDMMC_ICR register is a write-only register. Writing a bit with 1 clears the corresponding bit in the SDMMC_STAR status register.
    volatile uint32_t MASKR; // The interrupt mask register determines which status flags generate an interrupt request by setting the corresponding bit to 1.
    volatile uint32_t ACKTIMER; // The SDMMC_ACKTIMER register contains the acknowledgment timeout period, in SDMMC_CK bus clock periods. A counter loads the value from the SDMMC_ACKTIMER register, and starts decrementing when the data path state machine (DPSM) enters the Wait_Ack state. If the timer reaches 0 while the DPSM is in this states, the acknowledgment timeout status flag is set.
    reserved_t<0x3> _0x44;
    volatile uint32_t IDMACTRLR; // The receive and transmit FIFOs can be read or written as 32-bit wide registers. The FIFOs contain 32 entries on 32 sequential addresses. This allows the CPU to use its load and store multiple operands to read from/write to the FIFO.
    volatile uint32_t IDMABSIZER; // The SDMMC_IDMABSIZER register contains the buffers size when in double buffer configuration.
    volatile uint32_t IDMABASE0R; // The SDMMC_IDMABASE0R register contains the memory buffer base address in single buffer configuration and the buffer 0 base address in double buffer configuration.
    volatile uint32_t IDMABASE1R; // The SDMMC_IDMABASE1R register contains the double buffer configuration second buffer memory base address.
    reserved_t<0x8> _0x60;
    volatile uint32_t FIFOR; // The receive and transmit FIFOs can be only read or written as word (32-bit) wide registers. The FIFOs contain 16 entries on sequential addresses. This allows the CPU to use its load and store multiple operands to read from/write to the FIFO.When accessing SDMMC_FIFOR with half word or byte access an AHB bus fault is generated.
    reserved_t<0xdc> _0x84;
    volatile uint32_t VER; // SDMMC IP version register
    volatile uint32_t ID; // SDMMC IP identification register

    static constexpr uint32_t POWER_RESET_VALUE = 0x0; // Reset value
    typedef bit_field_t<0, 0x3> POWER_PWRCTRL; // SDMMC state control bits. These bits can only be written when the SDMMC is not in the power-on state (PWRCTRL?11). These bits are used to define the functional state of the SDMMC signals: Any further write will be ignored, PWRCTRL value will keep 11.
    ...
    typedef bit_field_t<0, 0x3ff> CLKCR_CLKDIV; // Clock divide factor This bit can only be written when the CPSM and DPSM are not active (CPSMACT = 0 and DPSMACT = 0). This field defines the divide factor between the input clock (SDMMCCLK) and the output clock (SDMMC_CK): SDMMC_CK frequency = SDMMCCLK / [2 * CLKDIV]. 0xx: etc.. xxx: etc..
    ...
    typedef bit_field_t<14, 0x3> CLKCR_WIDBUS; // Wide bus mode enable bit This bit can only be written when the CPSM and DPSM are not active (CPSMACT = 0 and DPSMACT = 0)
    ...
    typedef bit_field_t<20, 0x3> CLKCR_SELCLKRX; // Receive clock selection. These bits can only be written when the CPSM and DPSM are not active (CPSMACT = 0 and DPSMACT = 0)

    static constexpr uint32_t ARGR_RESET_VALUE = 0x0; // Reset value
    typedef bit_field_t<0, 0xffffffff> ARGR_CMDARG; // Command argument. These bits can only be written by firmware when CPSM is disabled (CPSMEN = 0). Command argument sent to a card as part of a command message. If a command contains an argument, it must be loaded into this register before writing a command to the command register.

    static constexpr uint32_t CMDR_RESET_VALUE = 0x0; // Reset value
    typedef bit_field_t<0, 0x3f> CMDR_CMDINDEX; // Command index. This bit can only be written by firmware when CPSM is disabled (CPSMEN = 0). The command index is sent to the card as part of a command message.
    ...
    typedef bit_field_t<8, 0x3> CMDR_WAITRESP; // Wait for response bits. This bit can only be written by firmware when CPSM is disabled (CPSMEN = 0). They are used to configure whether the CPSM is to wait for a response, and if yes, which kind of response.
    ...
    static constexpr uint32_t RESPCMDR_RESET_VALUE = 0xa3c5dd01; // Reset value
    ...
    typedef bit_field_t<0, 0xffffffff> RESP1R_CARDSTATUS1; // see Table 432

    static constexpr uint32_t RESP2R_RESET_VALUE = 0x0; // Reset value
    typedef bit_field_t<0, 0xffffffff> RESP2R_CARDSTATUS2; // see Table404.

    static constexpr uint32_t RESP3R_RESET_VALUE = 0x0; // Reset value
    typedef bit_field_t<0, 0xffffffff> RESP3R_CARDSTATUS3; // see Table404.

    static constexpr uint32_t RESP4R_RESET_VALUE = 0x0; // Reset value
    typedef bit_field_t<0, 0xffffffff> RESP4R_CARDSTATUS4; // see Table404.

    static constexpr uint32_t DTIMER_RESET_VALUE = 0x0; // Reset value
    typedef bit_field_t<0, 0xffffffff> DTIMER_DATATIME; // Data and R1b busy timeout period This bit can only be written when the CPSM and DPSM are not active (CPSMACT = 0 and DPSMACT = 0). Data and R1b busy timeout period expressed in card bus clock periods.

    static constexpr uint32_t DLENR_RESET_VALUE = 0x0; // Reset value
    typedef bit_field_t<0, 0x1ffffff> DLENR_DATALENGTH; // Data length value This register can only be written by firmware when DPSM is inactive (DPSMACT = 0). Number of data bytes to be transferred. When DDR = 1 DATALENGTH is truncated to a multiple of 2. (The last odd byte is not transfered) When DATALENGTH = 0 no data will be transfered, when requested by a CPSMEN and CMDTRANS = 1 also no command will be transfered. DTEN and CPSMEN are cleared to 0.
    ...
    typedef bit_field_t<2, 0x3> DCTRL_DTMODE; // Data transfer mode selection. This bit can only be written by firmware when DPSM is inactive (DPSMACT = 0).
    typedef bit_field_t<4, 0xf> DCTRL_DBLOCKSIZE; // Data block size This bit can only be written by firmware when DPSM is inactive (DPSMACT = 0). Define the data block length when the block data transfer mode is selected: When DATALENGTH is not a multiple of DBLOCKSIZE, the transfered data is truncated at a multiple of DBLOCKSIZE. (Any remain data will not be transfered.) When DDR = 1, DBLOCKSIZE = 0000 shall not be used. (No data will be transfered)
    ...
    typedef bit_field_t<0, 0x1ffffff> DCNTR_DATACOUNT; // Data count value When read, the number of remaining data bytes to be transferred is returned. Write has no effect.
    ...
    typedef bit_field_t<0, 0x1ffffff> ACKTIMER_ACKTIME; // Boot acknowledgment timeout period This bit can only be written by firmware when CPSM is disabled (CPSMEN = 0). Boot acknowledgment timeout period expressed in card bus clock periods.
    ...
    typedef bit_field_t<5, 0xff> IDMABSIZER_IDMABNDT; // Number of transfers per buffer. This 8-bit value shall be multiplied by 8 to get the size of the buffer in 32-bit words and by 32 to get the size of the buffer in bytes. Example: IDMABNDT = 0x01: buffer size = 8 words = 32 bytes. These bits can only be written by firmware when DPSM is inactive (DPSMACT = 0).

    static constexpr uint32_t IDMABASE0R_RESET_VALUE = 0x0; // Reset value
    typedef bit_field_t<0, 0xffffffff> IDMABASE0R_IDMABASE0; // Buffer 0 memory base address bits [31:2], shall be word aligned (bit [1:0] are always 0 and read only). This register can be written by firmware when DPSM is inactive (DPSMACT = 0), and can dynamically be written by firmware when DPSM active (DPSMACT = 1) and memory buffer 0 is inactive (IDMABACT = 1).

    static constexpr uint32_t IDMABASE1R_RESET_VALUE = 0x0; // Reset value
    typedef bit_field_t<0, 0xffffffff> IDMABASE1R_IDMABASE1; // Buffer 1 memory base address, shall be word aligned (bit [1:0] are always 0 and read only). This register can be written by firmware when DPSM is inactive (DPSMACT = 0), and can dynamically be written by firmware when DPSM active (DPSMACT = 1) and memory buffer 1 is inactive (IDMABACT = 0).


    static constexpr uint32_t FIFOR_RESET_VALUE = 0x0; // Reset value
    typedef bit_field_t<0, 0xffffffff> FIFOR_FIFODATA; // Receive and transmit FIFO data This register can only be read or written by firmware when the DPSM is active (DPSMACT=1). The FIFO data occupies 16 entries of 32-bit words.


    static constexpr uint32_t VER_RESET_VALUE = 0x10; // Reset value
    typedef bit_field_t<0, 0xf> VER_MINREV; // IP minor revision number.
    typedef bit_field_t<4, 0xf> VER_MAJREV; // IP major revision number.

    static constexpr uint32_t ID_RESET_VALUE = 0x140022; // Reset value
    typedef bit_field_t<0, 0xffffffff> ID_IP_ID; // SDMMC IP identification.
};

// SDMMC1: SDMMC1

struct stm32h745_cm4_sdmmc1_t
{
    volatile uint32_t POWER; // SDMMC power control register
    volatile uint32_t CLKCR; // The SDMMC_CLKCR register controls the SDMMC_CK output clock, the SDMMC_RX_CLK receive clock, and the bus width.
    volatile uint32_t ARGR; // The SDMMC_ARGR register contains a 32-bit command argument, which is sent to a card as part of a command message.
...
template<>
struct peripheral_t<STM32H723, SDMMC1>
{
...
template<>
struct peripheral_t<STM32H723, SDMMC2>
{
...
template<>
struct peripheral_t<STM32H73x, SDMMC2>
{
    static constexpr periph_t P = SDMMC2;
    using T = stm32h723_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H742x, SDMMC1>
{
    static constexpr periph_t P = SDMMC1;
    using T = stm32h742x_sdmmc1_t;
    static T& V;
};

template<>
struct peripheral_t<STM32H742x, SDMMC2>
{
    static constexpr periph_t P = SDMMC2;
    using T = stm32h742x_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H743, SDMMC1>
{
    static constexpr periph_t P = SDMMC1;
    using T = stm32h742x_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H743, SDMMC2>
{
    static constexpr periph_t P = SDMMC2;
    using T = stm32h742x_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H753, SDMMC1>
{
    static constexpr periph_t P = SDMMC1;
    using T = stm32h742x_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H753, SDMMC2>
{
    static constexpr periph_t P = SDMMC2;
    using T = stm32h742x_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H745_CM4, SDMMC1>
{
    static constexpr periph_t P = SDMMC1;
    using T = stm32h745_cm4_sdmmc1_t;
    static T& V;
};

template<>
struct peripheral_t<STM32H745_CM4, SDMMC2>
{
    static constexpr periph_t P = SDMMC2;
    using T = stm32h745_cm4_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H745_CM7, SDMMC1>
{
    static constexpr periph_t P = SDMMC1;
    using T = stm32h745_cm4_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H745_CM7, SDMMC2>
{
    static constexpr periph_t P = SDMMC2;
    using T = stm32h745_cm4_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H747_CM4, SDMMC1>
{
    static constexpr periph_t P = SDMMC1;
    using T = stm32h745_cm4_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H747_CM4, SDMMC2>
{
    static constexpr periph_t P = SDMMC2;
    using T = stm32h745_cm4_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H747_CM7, SDMMC1>
{
    static constexpr periph_t P = SDMMC1;
    using T = stm32h745_cm4_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H747_CM7, SDMMC2>
{
    static constexpr periph_t P = SDMMC2;
    using T = stm32h745_cm4_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H755_CM4, SDMMC1>
{
    static constexpr periph_t P = SDMMC1;
    using T = stm32h745_cm4_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H755_CM4, SDMMC2>
{
    static constexpr periph_t P = SDMMC2;
    using T = stm32h745_cm4_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H755_CM7, SDMMC1>
{
    static constexpr periph_t P = SDMMC1;
    using T = stm32h745_cm4_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H755_CM7, SDMMC2>
{
    static constexpr periph_t P = SDMMC2;
    using T = stm32h745_cm4_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H757_CM4, SDMMC1>
{
    static constexpr periph_t P = SDMMC1;
    using T = stm32h745_cm4_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H757_CM4, SDMMC2>
{
    static constexpr periph_t P = SDMMC2;
    using T = stm32h745_cm4_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H757_CM7, SDMMC1>
{
    static constexpr periph_t P = SDMMC1;
    using T = stm32h745_cm4_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H757_CM7, SDMMC2>
{
    static constexpr periph_t P = SDMMC2;
    using T = stm32h745_cm4_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H750x, SDMMC1>
{
    static constexpr periph_t P = SDMMC1;
    using T = stm32h750x_sdmmc1_t;
    static T& V;
};

template<>
struct peripheral_t<STM32H750x, SDMMC2>
{
    static constexpr periph_t P = SDMMC2;
    using T = stm32h750x_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H7A3x, SDMMC1>
{
    static constexpr periph_t P = SDMMC1;
    using T = stm32h750x_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H7A3x, SDMMC2>
{
    static constexpr periph_t P = SDMMC2;
    using T = stm32h750x_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H7B0x, SDMMC1>
{
    static constexpr periph_t P = SDMMC1;
    using T = stm32h750x_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H7B0x, SDMMC2>
{
    static constexpr periph_t P = SDMMC2;
    using T = stm32h750x_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H7B3x, SDMMC1>
{
    static constexpr periph_t P = SDMMC1;
    using T = stm32h750x_sdmmc1_t;
    static T& V;
...
template<>
struct peripheral_t<STM32H7B3x, SDMMC2>
{
    static constexpr periph_t P = SDMMC2;
    using T = stm32h750x_sdmmc1_t;
    static T& V;
