...
struct stm32h742x_sdmmc1_t
{
    ...
    template<uint32_t X>
    static constexpr uint32_t POWER_PWRCTRL =        // SDMMC state control bits. These bits can only be written when the SDMMC is not in the power-on state (PWRCTRL?11). These bits are used to define the functional state of the SDMMC signals: Any further write will be ignored, PWRCTRL value will keep 11.
    ...
    template<uint32_t X>
    static constexpr uint32_t CLKCR_WIDBUS =            // Wide bus mode enable bit This bit can only be written when the CPSM and DPSM are not active (CPSMACT = 0 and DPSMACT = 0)
    ...
    template<uint32_t X>
    static constexpr uint32_t CLKCR_SELCLKRX =          // Receive clock selection. These bits can only be written when the CPSM and DPSM are not active (CPSMACT = 0 and DPSMACT = 0)
    ...
    template<uint32_t X>
    static constexpr uint32_t CMDR_CMDINDEX =            // Command index. This bit can only be written by firmware when CPSM is disabled (CPSMEN = 0). The command index is sent to the card as part of a command message.
    ...
    template<uint32_t X>
    static constexpr uint32_t CMDR_WAITRESP =            // Wait for response bits. This bit can only be written by firmware when CPSM is disabled (CPSMEN = 0). They are used to configure whether the CPSM is to wait for a response, and if yes, which kind of response.
    ...
    template<uint32_t X>
    static constexpr uint32_t DCTRL_DTMODE =            // Data transfer mode selection. This bit can only be written by firmware when DPSM is inactive (DPSMACT = 0).
    ...
    template<uint32_t X>
    static constexpr uint32_t DCNTR_DATACOUNT =   // Data count value When read, the number of remaining data bytes to be transferred is returned. Write has no effect.
        bit_field_t<0, 0x1ffffff>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t ACKTIMER_ACKTIME =   // Boot acknowledgment timeout period This bit can only be written by firmware when CPSM is disabled (CPSMEN = 0). Boot acknowledgment timeout period expressed in card bus clock periods.
        bit_field_t<0, 0x1ffffff>::value<X>();
    ...
    static constexpr uint32_t FIFOR_RESET_VALUE = 0x0;

    static constexpr uint32_t VER_RESET_VALUE = 0x10;
    template<uint32_t X>
    static constexpr uint32_t VER_MINREV =   // IP minor revision number.
        bit_field_t<0, 0xf>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t VER_MAJREV =   // IP major revision number.
        bit_field_t<4, 0xf>::value<X>();

    static constexpr uint32_t ID_RESET_VALUE = 0x140022;

    static constexpr uint32_t RESPCMDR_RESET_VALUE = 0xa3c5dd01;
    template<uint32_t X>
...
struct stm32h750x_sdmmc1_t
{
    ...
    volatile uint32_t SDMMC_CLKCR;      // [read-write] The SDMMC_CLKCR register controls the SDMMC_CK output clock, the SDMMC_RX_CLK receive clock, and the bus width.
    ...
    volatile uint32_t SDMMC_RESPCMDR;   // [read-only] SDMMC command response register
    ...
    volatile uint32_t SDMMC_STAR;       // [read-only] The SDMMC_STAR register is a read-only register. It contains two types of flag:Static flags (bits [29,21,11:0]): these bits remain asserted until they are cleared by writing to the SDMMC interrupt Clear register (see SDMMC_ICR)Dynamic flags (bits [20:12]): these bits change state depending on the state of the underlying logic (for example, FIFO full and empty flags are asserted and de-asserted as data while written to the FIFO)
    ...
    volatile uint32_t SDMMC_IDMABSIZER; // [read-write] The SDMMC_IDMABSIZER register contains the buffers size when in double buffer configuration.
    volatile uint32_t SDMMC_IDMABASE0R; // [read-write] The SDMMC_IDMABASE0R register contains the memory buffer base address in single buffer configuration and the buffer 0 base address in double buffer configuration.
    volatile uint32_t SDMMC_IDMABASE1R; // [read-write] The SDMMC_IDMABASE1R register contains the double buffer configuration second buffer memory base address.
    reserved_t<0x8> _0x80;
    volatile uint32_t SDMMC_FIFOR;      // [read-write] The receive and transmit FIFOs can be only read or written as word (32-bit) wide registers. The FIFOs contain 16 entries on sequential addresses. This allows the CPU to use its load and store multiple operands to read from/write to the FIFO.When accessing SDMMC_FIFOR with half word or byte access an AHB bus fault is generated.
    reserved_t<0xdc> _0x3f4;
    volatile uint32_t SDMMC_VER;        // [read-only] SDMMC IP version register
    volatile uint32_t SDMMC_ID;         // [read-only] SDMMC IP identification register

    static constexpr uint32_t SDMMC_POWER_RESET_VALUE = 0x0;
    template<uint32_t X>
    static constexpr uint32_t SDMMC_POWER_PWRCTRL =        // SDMMC state control bits. These bits can only be written when the SDMMC is not in the power-on state (PWRCTRL?11). These bits are used to define the functional state of the SDMMC signals: Any further write will be ignored, PWRCTRL value will keep 11.
        bit_field_t<0, 0x3>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t SDMMC_CLKCR_WIDBUS =            // Wide bus mode enable bit This bit can only be written when the CPSM and DPSM are not active (CPSMACT = 0 and DPSMACT = 0)
        bit_field_t<14, 0x3>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t SDMMC_CLKCR_SELCLKRX =          // Receive clock selection. These bits can only be written when the CPSM and DPSM are not active (CPSMACT = 0 and DPSMACT = 0)
        bit_field_t<20, 0x3>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t SDMMC_CMDR_CMDINDEX =            // Command index. This bit can only be written by firmware when CPSM is disabled (CPSMEN = 0). The command index is sent to the card as part of a command message.
        bit_field_t<0, 0x3f>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t SDMMC_CMDR_WAITRESP =            // Wait for response bits. This bit can only be written by firmware when CPSM is disabled (CPSMEN = 0). They are used to configure whether the CPSM is to wait for a response, and if yes, which kind of response.
        bit_field_t<8, 0x3>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t SDMMC_DCTRL_DTMODE =            // Data transfer mode selection. This bit can only be written by firmware when DPSM is inactive (DPSMACT = 0).
        bit_field_t<2, 0x3>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SDMMC_DCTRL_DBLOCKSIZE =        // Data block size This bit can only be written by firmware when DPSM is inactive (DPSMACT = 0). Define the data block length when the block data transfer mode is selected: When DATALENGTH is not a multiple of DBLOCKSIZE, the transfered data is truncated at a multiple of DBLOCKSIZE. (Any remain data will not be transfered.) When DDR = 1, DBLOCKSIZE = 0000 shall not be used. (No data will be transfered)
        bit_field_t<4, 0xf>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t SDMMC_DCNTR_DATACOUNT =   // Data count value When read, the number of remaining data bytes to be transferred is returned. Write has no effect.
        bit_field_t<0, 0x1ffffff>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t SDMMC_ACKTIMER_ACKTIME =   // Boot acknowledgment timeout period This bit can only be written by firmware when CPSM is disabled (CPSMEN = 0). Boot acknowledgment timeout period expressed in card bus clock periods.
        bit_field_t<0, 0x1ffffff>::value<X>();
    ...
    template<uint32_t X>
    static constexpr uint32_t SDMMC_IDMABSIZER_IDMABNDT =   // Number of transfers per buffer. This 8-bit value shall be multiplied by 8 to get the size of the buffer in 32-bit words and by 32 to get the size of the buffer in bytes. Example: IDMABNDT = 0x01: buffer size = 8 words = 32 bytes. These bits can only be written by firmware when DPSM is inactive (DPSMACT = 0).
        bit_field_t<5, 0xff>::value<X>();

    static constexpr uint32_t SDMMC_IDMABASE0R_RESET_VALUE = 0x0;

    static constexpr uint32_t SDMMC_IDMABASE1R_RESET_VALUE = 0x0;

    static constexpr uint32_t SDMMC_FIFOR_RESET_VALUE = 0x0;

    static constexpr uint32_t SDMMC_VER_RESET_VALUE = 0x10;
    template<uint32_t X>
    static constexpr uint32_t SDMMC_VER_MINREV =   // IP minor revision number.
        bit_field_t<0, 0xf>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t SDMMC_VER_MAJREV =   // IP major revision number.
        bit_field_t<4, 0xf>::value<X>();

    static constexpr uint32_t SDMMC_ID_RESET_VALUE = 0x140022;

    static constexpr uint32_t SDMMC_RESPCMDR_RESET_VALUE = 0xa3c5dd01;
    template<uint32_t X>
    static constexpr uint32_t SDMMC_RESPCMDR_RESPCMD =   // Response command index
        bit_field_t<0, 0x3f>::value<X>();
};
...
{
    typedef stm32h745_cm4_sdmmc1_t T;
    static T& V;
};
...
template<> struct sdmmc_traits<2>
{
    ...
    static constexpr alternate_function_t CK = SDMMC2_CK;
    static constexpr alternate_function_t CMD = SDMMC2_CMD;
    static constexpr alternate_function_t D0 = SDMMC2_D0;
    static constexpr alternate_function_t D1 = SDMMC2_D1;
    static constexpr alternate_function_t D2 = SDMMC2_D2;
    static constexpr alternate_function_t D3 = SDMMC2_D3;
    static constexpr alternate_function_t D4 = SDMMC2_D4;
    static constexpr alternate_function_t D5 = SDMMC2_D5;
    static constexpr alternate_function_t D6 = SDMMC2_D6;
    static constexpr alternate_function_t D7 = SDMMC2_D7;
};
