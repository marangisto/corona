...
struct stm32l100_opamp_t
{
    ...
    volatile uint32_t OTR;   // [read-write] offset trimming register for normal mode
    volatile uint32_t LPOTR; // [read-write] OPAMP offset trimming register for low power mode

    static constexpr uint32_t CSR_RESET_VALUE = 0x10101;
    ...
    static constexpr uint32_t OTR_RESET_VALUE = 0x0;
    static constexpr uint32_t OTR_OT_USER = 0x80000000;   // Select user or factory trimming value
    template<uint32_t X>
    static constexpr uint32_t OTR_AO3_OPT_OFFSET_TRIM =   // OPAMP3, 10-bit offset trim value for normal mode
        bit_field_t<20, 0x3ff>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t OTR_AO2_OPT_OFFSET_TRIM =   // OPAMP2, 10-bit offset trim value for normal mode
        bit_field_t<10, 0x3ff>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t OTR_AO1_OPT_OFFSET_TRIM =   // OPAMP1, 10-bit offset trim value for normal mode
        bit_field_t<0, 0x3ff>::value<X>();

    static constexpr uint32_t LPOTR_RESET_VALUE = 0x0;
    template<uint32_t X>
    static constexpr uint32_t LPOTR_AO3_OPT_OFFSET_TRIM_LP =   // OPAMP3, 10-bit offset trim value for low power mode
        bit_field_t<20, 0x3ff>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t LPOTR_AO2_OPT_OFFSET_TRIM_LP =   // OPAMP2, 10-bit offset trim value for low power mode
        bit_field_t<10, 0x3ff>::value<X>();
    template<uint32_t X>
    static constexpr uint32_t LPOTR_AO1_OPT_OFFSET_TRIM_LP =   // OPAMP1, 10-bit offset trim value for low power mode
        bit_field_t<0, 0x3ff>::value<X>();
};
