...
struct stm32l552_pwr_t
{
    ...
    reserved_t<0x6> _0x78;
    volatile uint32_t SECCFGR;  // [read-write] Power secure configuration register
    reserved_t<0x1> _0x80;
    volatile uint32_t PRIVCFGR; // [read-write] Power privilege configuration register

    static constexpr uint32_t CR1_RESET_VALUE = 0x400;
    ...
    template<uint32_t X>
    static constexpr uint32_t CR3_RRS =                // SRAM2 retention in Standby mode
...
using sec_pwr_t = peripheral_t<mcu_svd, SEC_PWR>;
