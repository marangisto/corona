...
////
//
//      STM32U5 HASH peripherals
//
///

// HASH: Hash processor

struct stm32u5xx_hash_t
{
    ...
    typedef bit_field_t<17, 0x3> CR_ALGO; // Algorithm selection
    ...
    typedef bit_field_t<16, 0x1f> SR_NBWE; // Number of words expected
    static constexpr uint32_t SR_DINNE = 0x8000; // DIN not empty
    typedef bit_field_t<9, 0x1f> SR_NBWP; // Number of words already pushed
...
template<>
struct peripheral_t<STM32U5xx, HASH>
{
    static constexpr periph_t P = HASH;
    using T = stm32u5xx_hash_t;
    static T& V;
};

template<>
struct peripheral_t<STM32U5xx, SEC_HASH>
{
