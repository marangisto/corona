...
struct stm32wb30_cm4_dmamux_t
{
    ...
    volatile uint32_t CSR;   // [read-only] DMA Multiplexer Channel Status register
    volatile uint32_t CFR;   // [write-only] DMA Channel Clear Flag Register
    ...
    volatile uint32_t RGCFR; // [read-only] DMA Request Generator Clear Flag Register
...
{
    typedef stm32wb30_cm4_dmamux_t T;
    static T& V;
};
